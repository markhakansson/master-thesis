\section{Overview}
RAUK is a command-line interface (CLI) tool which be run on an existing
...
patches in custom forks of some libraries that are used by all RTIC
...
get the necessary data used for the schedulability analysis.
...
generally the case as it's recommended) then all read and writes will pass
...
Since it's supposed to run on the target hardware, it also executes the RTIC
...
the actual value is loaded into. 
