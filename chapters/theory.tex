...
are dispatched via one or more task queue dispatcher, where each dispatcher is
...
regards scheduling that RTIC provides, are zero-cost. This is because it lets
the hardware itself handle most of the scheduling work using ARM's nested
vector interrupt controller (NVIC). The overhead RTIC has, mostly concerns
resource management as well as software task dispatching.
